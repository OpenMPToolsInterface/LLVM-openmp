...
int main()
{
  ...
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_task_dependences:
  // CHECK-SAME: task_id=[[FIRST_TASK]], deps=[{{[0-9]+}}:type_inout, ], ndeps=1
  ...
  // CHECK-SAME: task_type=ompt_task_explicit=4, has_dependences=yes
  // CHECK: {{^}}[[MASTER_ID]]: ompt_event_task_dependences:
  // CHECK-SAME: task_id=[[SECOND_TASK]], deps=[{{[0-9]+}}:type_in, ], ndeps=1
