...
{
protected:
    std::map<ompd_address_space_context_t*, std::map<const char*, TType> > ttypes;
...
public:
  virtual std::string toString(){return std::string("TError messages not implemented yet");}
  friend TValue;
  friend TBaseValue;
};
...
public:  
   ...
   */
  TValue(ompd_address_space_context_t *_context, ompd_address_t _addr): TValue(_context, NULL, _addr) {}
  TValue(ompd_address_space_context_t *context, ompd_thread_context_t *tcontext, ompd_address_t addr);
//   TValue(ompd_address_space_context_t *context, const struct ompd_handle* th): TValue(context, NULL, th) {}
//   TValue(ompd_address_space_context_t *context, ompd_thread_context_t *tcontext, const struct ompd_handle* th);
  /**
...
//   ompd_rc_t getAddress(struct ompd_handle* th) const;
   ...
   * reduces the pointer level, uses the target value as new target address, keeps the target type
   */
...
//   ompd_rc_t getValue(struct ompd_handle* buf, int count);
  ...
  template<typename T>
  ompd_rc_t getValue(T& buf);

  friend TValue;
};

template<typename T>
ompd_rc_t TBaseValue::getValue(T& buf)
{
...
//EXTERN_C uint64_t getSystemThreadID(ompd_address_space_context_t *context, ompd_thread_handle_t t);
//EXTERN_C ompd_thread_handle_t getOmpThreadHandle(ompd_address_space_context_t *context);
//EXTERN_C void getThreadState(ompd_address_space_context_t *context, ompd_thread_handle_t t, ompt_state_t *state,
